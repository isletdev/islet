...
export const api = {
  ...
  provider: () => request<{ kind: string; configured: boolean; lastSnapshot: string; lastReason: string }>("/api/v1/provider"),
  providerSet: (kind: string, token: string) => post<{ kind: string; configured: boolean; lastSnapshot: string; lastReason: string }>("/api/v1/provider", { kind, token }),
  providerSnapshot: () => post<{ snapshot: string }>("/api/v1/provider/snapshot"),
