  ...
  return (
    <Card title={p.id ? `Settings for ${initial.name}` : "New runner pool"} description="Runners run inside Docker on this server. Jobs get a clean container each time.">
      <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Token" hint={p.id ? "Leave empty to keep the stored token." : pr.tokenHint}><Input type="password" value={p.token ?? ""} onChange={(e) => set({ token: e.target.value })} autoComplete="off" /></Field>
