...
function AdminerSetup({ s, onCancel, onSubmit }: { s: { suggestedHost: string; panelHost?: string; cookieDomain?: string }; onCancel: () => void; onSubmit: (v: { host: string; tls: string; protect: boolean }) => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ host: host.trim(), tls, protect }); }} className="mb-3 grid grid-cols-1 gap-3 rounded-md border border-border p-3 sm:grid-cols-2">
      ...
      {!cookieOk && <p className="text-xs text-warning sm:col-span-2">That guard needs the session cookie domain set to a parent of both the panel and this host{parent && <> (probably <span className="font-mono">{parent}</span>)</>}: <Link to="/settings?tab=team" className="text-accent hover:underline">Settings, Team, "Protect apps with Islet login"</Link>. Without it you will be sent to the login page in a loop.</p>}
