...
// Mark is how a connection without a row of its own is treated: read-only, and
// which environment it belongs to. Saved connections carry these as columns;
// a database Islet installed is listed at request time and has nowhere else to
// put them.
type Mark struct {
	Ref         string `json:"ref"`
	ReadOnly    bool   `json:"readOnly"`
...
// Marks returns every mark on this server, keyed by connection reference.
func (s *Store) Marks(ctx context.Context) (map[string]Mark, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT ref, read_only, environment FROM sql_marks WHERE server_id = ?`, s.serverID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var m Mark
		if err := rows.Scan(&m.Ref, &m.ReadOnly, &m.Environment); err != nil {
			return nil, err
		}
		out[m.Ref] = m
	}
...
// MarkFor returns one connection's mark, or the permissive default.
func (s *Store) MarkFor(ctx context.Context, ref string) Mark {
	m := Mark{Ref: ref, Environment: "development"}
	_ = s.db.QueryRowContext(ctx,
		`SELECT read_only, environment FROM sql_marks WHERE server_id = ? AND ref = ?`,
		s.serverID, ref).Scan(&m.ReadOnly, &m.Environment)
	return m
}

// SetMark records how a connection should be treated.
func (s *Store) SetMark(ctx context.Context, m Mark) error {
	switch m.Environment {
	case "development", "staging", "production":
	case "":
		m.Environment = "development"
	default:
		return fmt.Errorf("%q is not an environment: use development, staging or production", m.Environment)
	}
	_, err := s.db.ExecContext(ctx, `INSERT INTO sql_marks (server_id, ref, read_only, environment, updated_at)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT (server_id, ref) DO UPDATE SET read_only = excluded.read_only,
			environment = excluded.environment, updated_at = excluded.updated_at`,
		s.serverID, m.Ref, m.ReadOnly, m.Environment, s.stamp())
	return err
}
